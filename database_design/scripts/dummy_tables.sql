...
CREATE SCHEMA IF NOT EXISTS dem;  -- demand

-- 1) Shared types
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'road_class') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'lane_type') THEN
    CREATE TYPE lane_type  AS ENUM ('through','left','right','u_turn','bus','bike','parking','shoulder','aux');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'access_mode') THEN
    CREATE TYPE access_mode AS ENUM ('all','car','truck','bus','bike','ped');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'signal_control_type') THEN
    CREATE TYPE signal_control_type AS ENUM ('fixed_time','actuated','adaptive');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'phase_type') THEN
    CREATE TYPE phase_type AS ENUM ('permitted','protected','flashing');
  END IF;
END $$;
...
-- Demand first
DROP TABLE IF EXISTS dem.vehicle_trip CASCADE;
DROP TABLE IF EXISTS dem.path CASCADE;
DROP TABLE IF EXISTS dem.turning_flow CASCADE;
DROP TABLE IF EXISTS dem.od_entry CASCADE;
DROP TABLE IF EXISTS dem.od_matrix CASCADE;
DROP TABLE IF EXISTS dem.centroid_connector CASCADE;
DROP TABLE IF EXISTS dem.zone CASCADE;
...
CREATE TABLE net.node (
  ...
  geom           geometry(Point, 4326) GENERATED ALWAYS AS (
                    ST_SetSRID(ST_MakePoint(x, y), COALESCE(srid,4326))
                  ) STORED
);
...
CREATE TABLE net.movement (
  ...
  UNIQUE (via_node_id, from_link_id, to_link_id, COALESCE(from_lane_id,0), COALESCE(to_lane_id,0))
);
...
CREATE INDEX IF NOT EXISTS idx_zone_geom ON dem.zone USING GIST (geom);

-- Connectors between zone and network
CREATE TABLE dem.centroid_connector (
  connector_id   BIGSERIAL PRIMARY KEY,
  zone_id        BIGINT NOT NULL REFERENCES dem.zone(zone_id) ON DELETE CASCADE,
...
CREATE TABLE dem.od_entry (
  od_id          BIGINT NOT NULL REFERENCES dem.od_matrix(od_id) ON DELETE CASCADE,
  slice_index    INTEGER NOT NULL,
  o_zone_id      BIGINT NOT NULL REFERENCES dem.zone(zone_id) ON DELETE CASCADE,
  d_zone_id      BIGINT NOT NULL REFERENCES dem.zone(zone_id) ON DELETE CASCADE,
  demand_pcu     DOUBLE PRECISION NOT NULL,
  mode           access_mode DEFAULT 'car',
  PRIMARY KEY (od_id, slice_index, o_zone_id, d_zone_id, mode)
);
CREATE INDEX IF NOT EXISTS idx_odentry_lookup ON dem.od_entry (od_id, slice_index, o_zone_id, d_zone_id);

-- Turning movement demand tied to geometric movements
-- Store by discrete slice or exact timestamp
CREATE TABLE dem.turning_flow (
...
CREATE INDEX IF NOT EXISTS idx_tf_movement ON dem.turning_flow (movement_id);

-- Optional path storage
CREATE TABLE dem.path (
  path_id        BIGSERIAL PRIMARY KEY,
  od_id          BIGINT REFERENCES dem.od_matrix(od_id) ON DELETE SET NULL,
  slice_index    INTEGER,
  o_zone_id      BIGINT REFERENCES dem.zone(zone_id) ON DELETE SET NULL,
  d_zone_id      BIGINT REFERENCES dem.zone(zone_id) ON DELETE SET NULL,
  link_seq       BIGINT[]                       -- ordered net.link ids
);

-- Optional vehicle trips
CREATE TABLE dem.vehicle_trip (
  trip_id        BIGSERIAL PRIMARY KEY,
  depart_time    TIMESTAMP NOT NULL,
  origin_zone_id BIGINT REFERENCES dem.zone(zone_id),
  dest_zone_id   BIGINT REFERENCES dem.zone(zone_id),
  path_id        BIGINT REFERENCES dem.path(path_id),
...
-- Build movements, bind them to phases, then load dem.turning_flow for time slices.
